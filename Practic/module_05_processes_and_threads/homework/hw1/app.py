...
def get_pids(port: int) -> List[int]:
    ...
    commands = f"lsof -i :{port}"
    commands_split = shlex.split(commands)
    coomands_run = subprocess.run(commands_split, capture_output=True)

    out_read = coomands_run.stdout.decode()
    output_lines = out_read.splitlines()

    for line in output_lines[1:]:
        l_splt = line.split()
        pid_ = l_splt[1]
        pids.append(int(pid_))
