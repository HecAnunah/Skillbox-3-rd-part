...
def account(department: str, account_number: int):
    ...
    if dept_directory_name is None:
        logger.warning("Нет указанного департамента.")
    ...
    logger.debug(
        f"Пытаюсь прочитать профайл сотрудника с номером аккаунта {account_number}..."
    )
    with open(account_data_file, "r") as fi:
        account_data_txt = fi.read()
        logger.debug(f"Профайл сотрудника № {account_number} успешно загружен!")

    try:
        account_data_json = json.loads(account_data_txt)
    except Exception as e:
        logger.exception(f"Не удалась загрузка JSON-данных сотрудника", exc_info=e)
    ...
    try:
        ...
        logger.exception("Ошибка вывода данных о сотруднике", exc_info=exc)
        return "Invalid employee data", 500
